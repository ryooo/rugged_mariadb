...
def asplode(lib)
  if RUBY_PLATFORM =~ /mingw|mswin/
    abort "-----\n#{lib} is missing. Check your installation of MariaDB or Connector/C, and try again.\n-----"
  elsif RUBY_PLATFORM =~ /darwin/
    abort "-----\n#{lib} is missing. You may need to 'brew install mariadb' or 'port install mariadb', and try again.\n-----"
  else
    abort "-----\n#{lib} is missing. You may need to 'apt-get install libmariadbclient-dev' or 'yum install mariadb-devel', and try again.\n-----"
  end
end
...
def add_ssl_defines(header)
  ...
  has_no_support = all_modes_found ? false : !have_const('MARIADB_OPT_SSL_ENFORCE', header)
...
dirs = ENV.fetch('PATH').split(File::PATH_SEPARATOR) + %w(
  ...
  /opt/local
  /opt/local/mariadb
  /opt/local/lib/mariadb5*
  /usr
  /usr/mariadb
  /usr/local
  /usr/local/mariadb
  /usr/local/mariadb-*
  /usr/local/lib/mariadb5*
  /usr/local/opt/mariadb5*
).map { |dir| dir << '/bin' }

# For those without HOMEBREW_ROOT in PATH
dirs << "#{ENV['HOMEBREW_ROOT']}/bin" if ENV['HOMEBREW_ROOT']

GLOB = "{#{dirs.join(',')}}/{mariadb_config,m_config5,mariadb_config}"
...
create_makefile('rugged/mariadb/rugged_mariadb')
