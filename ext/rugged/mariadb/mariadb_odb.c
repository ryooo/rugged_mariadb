        ...
        git_odb_backend *_backend, const git_oid *oid)
{
    ...
    if (!backend->st_read_header
            && init_statement(backend->db, "read_header", SQL_READ_HEADER, backend->table,
                &backend->st_read_header) != GIT_OK)
        return GIT_EUSER;
    ...
    git_otype *type_p, git_odb_backend *_backend, const git_oid *oid)
{
    ...
    if (!backend->st_read
            && init_statement(backend->db, "read", SQL_READ, backend->table,
                &backend->st_read) != GIT_OK)
        return GIT_EUSER;
...
static int mariadb_odb_backend__read_prefix(
    ...
    *data_p = NULL;
    ...
    if (!backend->st_read_prefix
            && init_statement(backend->db, "read prefix", SQL_READ_PREFIX, backend->table,
                &backend->st_read_prefix) != GIT_OK)
        return GIT_EUSER;
...
static int mariadb_odb_backend__exists(git_odb_backend *_backend, const git_oid *oid)
{
    ...
    if (!backend->st_read_header
            && init_statement(backend->db, "read header", SQL_READ_HEADER, backend->table,
                &backend->st_read_header) != GIT_OK)
        return GIT_EUSER;
        ...
        const git_oid *short_oid, size_t len)
{
    ...
    if (!backend->st_read_header_prefix
            && init_statement(backend->db, "read header prefix", SQL_READ_HEADER_PREFIX, backend->table,
                &backend->st_read_header_prefix) != GIT_OK)
        return GIT_EUSER;
...
printf (">> mariadb_odb_backend__write\n");
    ...
    if (!backend->st_write
            && init_statement(backend->db, "write", SQL_WRITE, backend->table,
                &backend->st_write) != GIT_OK)
        return GIT_EUSER;
