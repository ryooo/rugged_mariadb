        ...
        const char *refname)
{
    ...
    if (!backend->st_exists
            && init_statement(backend->db, "exists", SQL_EXISTS, backend->table,
        ...
        const char *refname)
{
    ...
    *out = NULL;
    ...
    if (!backend->st_lookup
            && init_statement(backend->db, "lookup", SQL_LOOKUP, backend->table,
        ...
        struct git_refdb_backend *_backend, const char *glob)
{
    ...
    *_iterator = NULL;
    ...
    if (!backend->st_iterator
            && init_statement(backend->db, "iterator", SQL_ITERATOR, backend->table,
        ...
        const git_oid *old, const char *old_target)
{
    ...
    if (force) {
        if (!backend->st_write_force
                && init_statement(backend->db, "write force", SQL_WRITE_FORCE,
                    backend->table, &backend->st_write_force) != GIT_OK)
            return GIT_EUSER;

        sql_statement = backend->st_write_force;
    } else {
        if (!backend->st_write_no_force
                && init_statement(backend->db, "write force", SQL_WRITE_FORCE,
                    backend->table, &backend->st_write_no_force) != GIT_OK)
            return GIT_EUSER;

        sql_statement = backend->st_write_no_force;
    }
        ...
        const git_signature *who, const char *message)
{
    ...
    *out = NULL;
    ...
    if (!backend->st_rename
            && init_statement(backend->db, "rename", SQL_RENAME, backend->table,
        ...
        const git_oid *old_id, const char *old_target)
{
    ...
    if (!backend->st_delete
            && init_statement(backend->db, "delete", SQL_DELETE, backend->table,
...
static int mariadb_refdb_compress(git_refdb_backend *_backend)
{
    ...
    if (!backend->st_optimize
            && init_statement(backend->db, "optimize", SQL_OPTIMIZE, backend->table,
